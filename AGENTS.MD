...
- **Current state**:
  ...
  - Automatic Danbooru metadata enrichment on upload (rating/source/tags) via `sources/danbooru/*` with in-process caching.
  ...
  - Automatic image-family creation uses crop-aware pHash candidate retrieval followed by OpenCV SIFT/RANSAC verification; existing families are parent-gated and never auto-merged.
  - Tag management (associate/dissociate) and recommendations (local + Danbooru).
...
- **Planned/expected** (not yet implemented):
  - Auth flows (signup/login, token issuance) and object storage integration.
...
- `server/api/`: API routers (`health.py`, `files.py`, `media.py`, `tags.py`).
- `server/api/serializers/`: Pydantic serializers (e.g., `file.py`, `danbooru.py`).
- `server/utils/`: Utilities (`file_storage.py`, `file_info.py`, `thumbnail_gen.py`).
- `server/sources/`: External integrations (Danbooru, ONNX models).
...
- **First boot**: The API may download the ONNX tagging model and tag list from Hugging Face before it becomes ready. This can take several seconds or longer depending on the connection, and Nginx may return `502 Bad Gateway` during initialization. Follow `docker compose logs -f api` and wait for `Application startup complete`; subsequent boots reuse the `ml_models` volume.
...
- Perceptual hashes use PostgreSQL `BIT(64)` and 64-character bit strings in Python. The conversion migration preserves existing signed `BIGINT` hash patterns; apply migrations before starting the updated application.
- To rebuild an existing library, stop the API/media writers, back up the DB, apply Alembic migrations, and run `uv run python v2_autofamily_migration.py --dry-run` from `server/`, then repeat with `--apply`. It resets all hashes and all families (including manual/video relationships), recomputes hashes for every image, then replays v2 autofamily in SHA256 order. It never changes media, tags, or pools. The rebuild holds write-blocking table locks and commits once; errors or dry runs roll everything back. A failed run must be restarted, not resumed.

### Current database schema

- **files**: sha256_hash (PK), md5_hash, file_size, original_filename, file_ext, file_type, width, height, rating enum, date_added, source, ai_generated, phash, phash_center_80, phash_center_50, processing_status, processing_error, parent_family_id
...
- **ONNX models**: Use `OnnxModel` from `server.ml.model` to download and cache ONNX models from Hugging Face Hub.
- **Storage**: Models are stored in `server/ml/blobs/<sha256>/<filename>` (configurable via `ML_MODELS_DIR` env var).
- **Persistence**: Docker Compose uses a named volume `ml_models` mounted at `/app/server/ml/blobs` to persist models between builds.
- **SHA-256**: Remote SHA-256 is fetched from Hugging Face metadata when available; otherwise computed after download and saved to `<filename>.sha256` for future checks.

Example usage:
```python
from server.ml.model import OnnxModel

# Model is automatically downloaded if not already present
onnx = OnnxModel(
    repo_id="org/repo",
    filename="path/to/model.onnx",
    revision="main"  # Optional, defaults to None
)
local_path = onnx.local_path  # Path to the downloaded model
```
...
- There is no formal test suite yet. For now:
  ...
  - **Upload**: `PUT /api/upload/file` with an image/video; confirm thumbnail created and Danbooru enrichment (when MD5 matches a Danbooru post).
...
curl -X PUT -F "file=@test.jpg" http://localhost:8000/api/upload/file | jq
...
- `PUT /api/upload/file` — Upload image/video from multipart form data; validates type; computes hashes; extracts dimensions; persists; auto-enriches from Danbooru and ONNX model.
- `POST /api/upload/url` — Upload image/video from a direct media URL or Danbooru post URL; resolves Danbooru posts to their original file, downloads to temp storage, validates type, computes hashes, extracts dimensions, persists, and auto-enriches from Danbooru and the ONNX model.
...
- `POST /api/files/{sha256}/refetch-danbooru` — Re-run Danbooru enrichment for a stored file (used when tags came from the ONNX model); replaces existing tags on a hit, returns 404 and leaves metadata untouched on a miss.
...
- `GET /api/tags/recommend` — Recommend tags from local usage.
- `GET /api/tags/danbooru-recs` — Recommend tags from Danbooru.
- `POST /api/tags/associate` — Associate a tag with a file.
- `DELETE /api/tags/dissociate` — Remove a tag from a file.

### Notes for future work

- Align Python versions (Docker image vs. `pyproject.toml`).
- Add test suite (pytest, coverage) and CI.
- Implement auth and object storage integration.

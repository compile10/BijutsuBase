<script lang="ts">
	...
	let {
		...
		isOpen?: boolean;
		familyId: string;
		onChildAdded?: (family: FileFamilyResponse) => void;
	}>();
	...
	async function handleAdd() {
		...
		try {
			const updatedFamily = await addChildToFamily(familyId, previewFile.sha256_hash);
			onChildAdded?.(updatedFamily);
			handleClose();
		} catch (err) {
			actionError = err instanceof Error ? err.message : 'Failed to add image to family.';
		} finally {
			isAdding = false;
		}
	}
...
<WindowModal bind:isOpen title="Add by SHA-256" maxWidth="max-w-2xl" onClose={handleClose}>
	<div class="flex items-center justify-between border-b border-gray-200 p-6 dark:border-gray-700">
		<h2 class="text-xl font-semibold text-gray-900 dark:text-white">Add Child by Hash</h2>
		<button
			onclick={handleClose}

						...
						>
							{#if thumb.thumbnail_url}
								<img
									...
									alt="Thumbnail"
									class="h-full w-full object-cover {isSelectMode && isSelected
										? 'opacity-75'
										: ''}"
								/>
							{:else if thumb.processing_status === 'failed'}
								<!-- Processing failed -->
								<div
									class="flex h-full w-full flex-col items-center justify-center text-red-400 dark:text-red-500"
								>
							...
							{:else}
								...
								<div
									class="flex h-full w-full flex-col items-center justify-center text-gray-400 dark:text-gray-500"
								>
							...
							{#if isSelectMode}
								<div class="absolute right-2 top-2 z-10">
									{#if isSelected}
										<IconCheckCircle
											class="h-6 w-6 text-primary-600 bg-white rounded-full dark:text-primary-400 dark:bg-gray-900"
										/>
									{:else}
							...
							{:else if thumb.thumbnail_url}
								<div
									class="absolute inset-0 bg-black opacity-0 transition-opacity group-hover:opacity-10"
								></div>
							{/if}
						</button>
					{/each}
...
<!-- Lightbox -->
<Lightbox bind:isOpen={lightboxOpen} bind:currentIndex={lightboxIndex} {files} />
